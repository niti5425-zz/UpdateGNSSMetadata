...
def parseArguments():
    global args_parser
    parser = argparse.ArgumentParser(usage = 'Generate domains for GNSS metadata fields (FixType, StationId, Number of Satellites')
    ...
    parser.add_argument('-r','--remove',default = False, type=bool, help='Set true if GNSS metadata fields need to be removed')
...
def generateToken():
    ...
    datapayload = bytes(urllib.parse.urlencode( \
        {'username':args_parser.username,'password':args_parser.password,'referer':args_parser.url,'f':'json','expiration':60}), 'utf-8')
    token_response = urllib.request.urlopen(urllib.parse.urljoin(args_parser.url,'/sharing/rest/GenerateToken?'),datapayload).read()
...
def searchItems_addGNSSMetadataFields():
    ...
    try:
        ...
        for id in itemId:
            ...
            featureLayerResponse = urllib.request.urlopen(urllib.parse.urljoin(featureServiceUrl,'?&f=pjson'),datapayload).read()
            ...
            if not args_parser.remove:
                ...
                if len(altitudeField) == 0:
                    gnssMetadataFields['fields'].append({'name':'ESRIGNSS_ALTITUDE', 'type': 'esriFieldTypeDouble',  \
                                                         'alias': 'Altitude', 'sqlType': 'sqlTypeOther','nullable': True, 'editable': True, \
                                                         'domain': None, 'defaultValue': None})
                # ESRIGNSS_PDOP
                ...
                if len(pdopField) == 0:
                    gnssMetadataFields['fields'].append({'name':'ESRIGNSS_PDOP', 'type': 'esriFieldTypeDouble',  \
                                                         'alias': 'PDOP', 'sqlType': 'sqlTypeOther','nullable': True, 'editable': True, \
                                                         'domain': None, 'defaultValue': None})
                ...
                if len(hdopField) == 0:
                    gnssMetadataFields['fields'].append({'name':'ESRIGNSS_HDOP', 'type': 'esriFieldTypeDouble',  \
                                                         'alias': 'HDOP', 'sqlType': 'sqlTypeOther','nullable': True, 'editable': True, \
                                                         'domain': None, 'defaultValue': None})
                ...
                if len(vdopField) == 0:
                    gnssMetadataFields['fields'].append({'name':'ESRIGNSS_VDOP', 'type': 'esriFieldTypeDouble',  \
                                                         'alias': 'VDOP', 'sqlType': 'sqlTypeOther','nullable': True, 'editable': True, \
                                                         'domain': None, 'defaultValue': None})

                # ESRIGNSS_CORRECTIONAGE
                correctionAgeField = [field for field in featureLayerFields if field['name'] == 'ESRIGNSS_CORRECTIONAGE']

                if len(correctionAgeField) == 0:
                    gnssMetadataFields['fields'].append({'name':'ESRIGNSS_CORRECTIONAGE', 'type': 'esriFieldTypeDouble',  \
                ...
                if len(fixTypeField) == 1:
                    ...
                    if fixTypeField[0]['domain'] == None:
                        ...
                        fixTypeDomain = {'type':'codedValue','name':'ESRI_FIX_TYPE_DOMAIN','codedValues':[{'name':'Fix not valid','code':0}, \
                                                                                                          {'name':'GPS','code':1},{'name':'Differential GPS', 'code':2}, \
                                                                                                          {'name':'RTK Fixed','code': 4},{'name':'RTK Float','code':5}]}
                    ...
                    gnssMetadataFields['fields'].append({'name':'ESRIGNSS_FIXTYPE', 'type': 'esriFieldTypeInteger',  \
                                                         'alias': 'Fix Type', 'sqlType': 'sqlTypeOther','nullable': True, 'editable': True, \
                                                         'domain': {'type':'codedValue','name':'ESRI_FIX_TYPE_DOMAIN','codedValues': \
                                                                    [{'name':'Fix not valid','code':0},{'name':'GPS','code':1},{'name':'Differential GPS', 'code':2}, \
                                                                     {'name':'RTK Fixed','code': 4},{'name':'RTK Float','code':5}]}, 'defaultValue': None})
                                             
                # ESRIGNSS_STATIONID
                stationIdField = [field for field in featureLayerFields if field['name'] == 'ESRIGNSS_STATIONID']

                if len(stationIdField) == 1:
                    # Field does exist check if the domain is set.
                    ...
                    gnssMetadataFields['fields'].append({'name':'ESRIGNSS_STATIONID', 'type': 'esriFieldTypeInteger',  \
                                                         'alias': 'Station ID', 'sqlType': 'sqlTypeOther','nullable': True, 'editable': True, \
                                                         'domain': {'type':'range','name':'ESRI_STATION_ID_DOMAIN','range':[0,1023]}, 'defaultValue': None})

                # ESRIGNSS_NUMSATS
                numstatsField = [field for field in featureLayerFields if field['name'] == 'ESRIGNSS_NUMSATS']

                if len(numstatsField) == 1:
                    # Field does exist check if the domain is set.
                    ...
                    gnssMetadataFields['fields'].append({'name':'ESRIGNSS_NUMSATS', 'type': 'esriFieldTypeInteger',  \
                                                         'alias': 'Number of Satellites', 'sqlType': 'sqlTypeOther','nullable': True, 'editable': True, \
                                                         'domain': {'type':'range','name':'ESRI_NUM_SATS_DOMAIN','range':[0,99]}, 'defaultValue': None})
                
                # Check if AddToDefinition operation needs to be added.
                initialFeatureLayerFieldsCount = len(featureLayerFields)
                if (len(gnssMetadataFields['fields']) + initialFeatureLayerFieldsCount) > initialFeatureLayerFieldsCount:
                    operations.append('addToDefinition')                               

            else:
                operations.append('deleteFromDefinition')
                gnssMetadataFields = {'fields': [{'name': 'ESRIGNSS_RECEIVER'},{'name': 'ESRIGNSS_H_RMS'},{'name':'ESRIGNSS_V_RMS'},{'name':'ESRIGNSS_LATITUDE'} , \
                                                 {'name': 'ESRIGNSS_LONGITUDE'},{'name':'ESRIGNSS_ALTITUDE'},{'name':'ESRIGNSS_PDOP'},{'name':'ESRIGNSS_HDOP'}, \
                                                 {'name': 'ESRIGNSS_VDOP'},{'name':'ESRIGNSS_CORRECTIONAGE'},{'name':'ESRIGNSS_FIXTYPE'},{'name':'ESRIGNSS_STATIONID'}, \
            ...
            for operation in operations:
                ...
                adminUrl = featureServiceUrl[0:servicesKeywordIndex] +'/admin' + featureServiceUrl[servicesKeywordIndex:] +'/'+operation+'?'
            
                # Update/Add/Delete Service Definition
                payload = ''
                if operation == 'addToDefinition' or operation == 'deleteFromDefinition':
                    payload = bytes(urllib.parse.urlencode({'token':token, 'f':'json', operation:json.dumps(gnssMetadataFields).encode('utf8', errors='replace')}),'utf-8')
                else:
                    featureLayerJson['editingInfo'] = {'lastEditDate' : ''}
                    payload = bytes(urllib.parse.urlencode({'token':token, 'f':'json', operation:json.dumps(featureLayerJson).encode('utf8', errors='replace')}),'utf-8')
